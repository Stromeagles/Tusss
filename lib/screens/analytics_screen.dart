...
class _ProgressAnalyticsScreenState extends State<ProgressAnalyticsScreen> {
  ...
  // ── Readiness Gauge (yeni — shared ReadinessCard) ──────────────────────────
  Widget _buildReadinessGauge(bool isDark) {
    return FutureBuilder<List<_SubjectMasteryData>>(
      ...
      builder: (context, snapshot) {
        ...
        final totalCards = masteryData.fold(0, (sum, s) => sum + s.total);
        final masteredCards = masteryData.fold(0, (sum, s) => sum + s.done);
        final masteryRatio =
            totalCards > 0 ? (masteredCards / totalCards).clamp(0.0, 1.0) : 0.0;

        final dailyRatio = widget.progress.dailyGoal > 0
            ? (widget.progress.todayStudied / widget.progress.dailyGoal)
                .clamp(0.0, 1.0)
            : 0.0;
        final streakRatio =
            (widget.progress.currentStreak / 30.0).clamp(0.0, 1.0);
        ...
        return ReadinessCard(
          ...
          scoreIntensity: widget.progress.scoreIntensity,
          targetScore: widget.progress.targetScore,
          recommendedDailyGoal: widget.progress.recommendedDailyGoal,
          masteryTarget: rd.masteryTarget,
          currentMastery: rd.currentMastery,
          daysToExam: widget.progress.daysToExam,
          isDark: isDark,
        );
      },
    );
